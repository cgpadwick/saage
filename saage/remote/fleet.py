...
class ThunderBackend:
    """Thunder Compute (tnr CLI). Cheap prototyping boxes (~$0.35/hr a6000);
    per-instance ssh keys; non-standard ports; tmux installed post-boot."""
    name = "thunder"
    HOURLY = {"a6000": 0.35}            # display estimate only
    ...
    def spawn(self, name: str, gpu: str, slots: int) -> Target:
        ...
        conn = SSHConn(host=inst.ip, user="ubuntu", key=key_path, port=inst.port)
        _wait_conn(conn)
        ...
        add_target(name, inst.ip, user="ubuntu", port=inst.port,
                   hourly_usd=self.HOURLY.get(gpu, 0.0), key=str(key_path),
                   max_runs=slots)
        return list_targets()[name]
