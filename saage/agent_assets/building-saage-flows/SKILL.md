...
- Provider: omit the `provider:` block (the norm — runs then use the user's `saage setup` defaults); pin `provider: { type: ..., model: ... }` only when the flow needs a specific model, and note `--provider/--model` still override at run time.
