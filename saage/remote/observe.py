...
def fetch_run(rs: RunState, dest: Path | None = None, *,
              via_bucket: bool = False,
              workspace: bool = False) -> tuple[Path, list[str], str]:
    ...
    `workspace=True` also pulls the run's workspace (generated source + .git
    history, minus venv/data/weights — see WS_FETCH_EXCLUDES); it needs the
    live node, so a mirror fallback silently skips it (the CLI notes this).
    """
    node = _node_for(rs)
...
def fetch(run_ref: str | None, dest: str | None = None, *,
    ...
    out, got, source = fetch_run(rs, Path(dest) if dest else None,
                                 via_bucket=via_bucket, workspace=workspace)
    if workspace and source != "node":
