...
def _run_body(spec: RunSpec, engine_line: str) -> str:
...
def start_sh(spec: RunSpec) -> str:
    ...
    body = _run_body(spec, engine)
    # inject the RUN_BRANCH_FLAG setup immediately before the engine line
    run_branch_block = (
        '\n# packaged workspace: tell the flow which branch it is on, so flows that manage\n'
        '# their own experiment branch (e.g. lewm setup_experiment --branch) commit to\n'
        '# the branch the push-back channel actually pushes\n'
    ...
    # insert before "status running\n" which precedes the engine line
    return body.replace('\nstatus running\n', run_branch_block + '\nstatus running\n', 1)
